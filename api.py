...
## 'cal' doesn't seem to work in terminal
...
@app.route('/pythonver', methods=['GET'])
def get_python_ver():
    result = subprocess.check_output(['python', '--version']).decode('utf-8')
    return jsonify({'python': result.strip()})
